...
干净、高效、可视化的自托管中心化 SSH Web 网关 · 版本 v0.0.7-2 · 更新日期 2026-09-14
...
- **服务器卡片视图**：桌面空白处右键选择「卡片模式」，服务器以卡片形式展示在线状态、系统 Logo、国家/地区国旗、微型监控、**价格**与**到期时间**；卡片可用左键拖动到自定义位置。到期时间与价格（金额 + 币种 + 计费周期）可在新增/编辑服务器时设置，临近到期（7 天内）或已过期会高亮提醒。
- **文件上传**：文件管理器支持直接把本地文件拖拽到窗口内上传（拖到文件夹行可上传到该文件夹），也支持「上传」按钮多选文件；左侧「快速访问」中的文件夹可上下拖动调整顺序。
- **网站管理**：可视化建站（静态 / 反向代理 / 重定向）与 Let's Encrypt 证书签发；一张证书可同时签发**多个域名（SAN）**与**泛域名**（如 `*.example.com`）。泛域名只能使用 DNS 验证（HTTP-01 无法验证泛域名），面板会自动切换。
- **任务栏与应用中心**：底部任务栏管理已打开窗口，🪟 按钮打开应用中心（添加服务器、设置、直链下载、一键命令、网站管理、卡片模式）。
...
- **自定义登录路由**：可在「设置 → 安全」修改登录页地址（如 `/secret-admin`）。修改后访问首页**不会**自动跳转到该地址（只显示「路由错误」），必须手动输入完整地址（形如 `#/secret-admin`）才能打开登录页。该地址**不会通过任何公开接口下发**（前端只按当前路径询问是否放行），并带枚举限流。
