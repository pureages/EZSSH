...
干净、高效、可视化的自托管中心化 SSH Web 网关 · 版本 v0.0.6-2 · 更新日期 2026-08-09
...
- **任务栏与应用中心**：底部任务栏管理已打开窗口，🪟 按钮打开应用中心（世界地图、添加服务器、设置、直链下载、一键命令、网站管理、pi-SSH-Agent）。
...
<a id="s7"></a>
## 7. pi-SSH-Agent（AI 管理服务器）

pi-SSH-Agent 是基于 [pi](https://github.com/earendil-works/pi) 的 AI 助手应用：打开网关本机的终端并自动启动 `pissh`（预置了 EZSSH SSH 扩展的 pi），你可以直接用自然语言让 AI 管理网关中已纳管的服务器。凭证由 EZSSH 保险库统一保管，AI 通过受限的 agent 接口按主机名执行命令，无需把密码交给 AI。

**使用步骤**：

1. **前提**：网关本机（Local/Gateway）已配置 SSH 账号密码。
2. 打开「应用中心 → pi-SSH-Agent」：
   - 若网关未安装 `pissh`，页面会显示一条一键安装命令（已内置访问令牌），到网关服务器终端执行即可；
   - 安装完成后重新打开应用，即进入网关本机终端并自动执行 `pissh`，进入 AI 聊天界面。
3. 首次使用需在 `pissh` 中配置模型（`/login` 或 provider API key）。
...
AI 会先调用 `list_hosts` 确认主机，再用 `ssh_exec` 在 `home99` 上执行命令并返回结果。

**内置工具**：

- `list_hosts`：列出 EZSSH 网关中保存的所有服务器（名称、地址、端口、用户、平台）。
- `ssh_exec`：在指定主机（名称或 ID）上执行 shell 命令并返回输出。

**卸载**：`pissh` 随 EZSSH 主程序一起卸载（卸载 EZSSH 时会自动删除 `/usr/local/bin/pissh`、pi 扩展与配置）。
